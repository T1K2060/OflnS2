...
(function () {
  ...
  const el = {
    ...
    versionCurrent:     document.getElementById('versionCurrent'),
    versionRequired:    document.getElementById('versionRequired'),
    ...
    storageModalClose:  document.getElementById('storageModalClose'),
    storageClearAll:    document.getElementById('storageClearAll'),
  ...
  async function loadLocalVersion() {
    try {
      ...
      console.log('[Version] Loaded local version:', version.hubVersion);
      return version;
    } catch (error) {
      console.warn('[Version] Failed to load local version:', error.message);
      return { hubVersion: '1.6.0' }; // Fallback version
    }
  }
  ...
  async function fetchAnnouncementConfig() {
    ...
    try {
      const response = await fetch('https://raw.githubusercontent.com/T1K2060/OflnSC/refs/heads/main/config.json');
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const config = await response.json();
      console.log('[Config] Loaded from GitHub raw URL');
      return config;
    } catch (error) {
      console.warn('[Config] GitHub raw URL failed, trying jsDelivr backup:', error.message);
      
      // Fallback to jsDelivr
      try {
        const response = await fetch('https://cdn.jsdelivr.net/gh/T1K2060/OflnSC@main/config.json');
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const config = await response.json();
        console.log('[Config] Loaded from jsDelivr backup');
        return config;
      } catch (fallbackError) {
        console.error('[Config] Both GitHub raw URL and jsDelivr failed:', fallbackError.message);
        return null;
      }
    }
  }
  ...
  function showAnnouncement(text) {
    ...
    el.announcementText.textContent = text.trim();
  ...
  // Start periodic announcement and update checking
  function startPeriodicChecking() {
    ...
    announcementCheckInterval = setInterval(async () => {
      try {
        ...
        if (config.announcement && config.announcement !== lastAnnouncement) {
          console.log('[Announcement] New announcement detected:', config.announcement);
          lastAnnouncement = config.announcement;
          showAnnouncement(config.announcement);
        }
        
        // Check for version updates
        if (config.requiredVersion && config.requiredVersion !== lastRequiredVersion) {
          console.log('[Update] New required version detected:', config.requiredVersion);
          lastRequiredVersion = config.requiredVersion;
          
          // Get local version and check for mismatch
          const localVersion = await loadLocalVersion();
          if (localVersion.hubVersion && localVersion.hubVersion !== config.requiredVersion) {
            console.log('[Update] Version mismatch detected, showing popup');
            showUpdatePopup(localVersion.hubVersion, config.requiredVersion);
          }
        }
      } catch (error) {
        console.warn('[Periodic Check] Failed to check for updates:', error.message);
      }
    }, 30000);
    
    console.log('[Periodic Check] Started announcement and update checking');
  }
  ...
  function stopPeriodicChecking() {
    if (announcementCheckInterval) {
      ...
      console.log('[Periodic Check] Stopped announcement and update checking');
    }
  }

  function hideAnnouncement() {
    el.announcementBanner.classList.add('fade-out');
    setTimeout(() => {
      el.announcementBanner.style.display = 'none';
      el.announcementBanner.classList.remove('fade-out');
    }, 500);
  }
  ...
  async function initAnnouncementSystem() {
    ...
    const config = await fetchAnnouncementConfig();
    if (!config) {
      // If GitHub fails, still show local version
      if (localVersion.hubVersion) {
        document.title = `TKHub - ${localVersion.hubVersion}`;
        if (el.sidebarVersion) {
          el.sidebarVersion.textContent = `v${localVersion.hubVersion}`;
        }
      }
      return;
    }
    ...
    // Update page title with local version
    if (localVersion.hubVersion) {
      document.title = `TKHub - ${localVersion.hubVersion}`;
    }

    // Update sidebar version display with local version
    if (localVersion.hubVersion && el.sidebarVersion) {
      el.sidebarVersion.textContent = `v${localVersion.hubVersion}`;
    }

    // Check if local version differs from GitHub RequiredVersion
    console.log('Version check:', localVersion.hubVersion, config.requiredVersion, localVersion.hubVersion !== config.requiredVersion);
    if (localVersion.hubVersion && config.requiredVersion && localVersion.hubVersion !== config.requiredVersion) {
  ...
  function showUpdatePopup(currentVersion, requiredVersion) {
    if (el.versionCurrent) el.versionCurrent.textContent = currentVersion;
    if (el.versionRequired) el.versionRequired.textContent = requiredVersion;
    if (el.updatePopup) el.updatePopup.style.display = 'flex';
  }

  function hideUpdatePopup() {
    if (el.updatePopup) el.updatePopup.style.display = 'none';
  }
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith(STORAGE_PREFIX)) {
        try {
          ...
          const gameName = state.games.find(g => g.filename === gameId)?.name || gameId;
          ...
          savedGames.push({
            ...
            size: formattedSize,
            date: date,
            keys: Object.keys(data.data || {}).length
          });
        } catch (e) {
          console.warn('Failed to parse storage data:', key, e);
        }
      }
    }
    ...
    if (savedGames.length === 0) {
      el.storageList.innerHTML = `
        <div class="storage-empty">
          ...
          <p>No saved game data found</p>
        </div>
      `;
    } else {
      el.storageList.innerHTML = savedGames.map(game => `
        <div class="storage-item" data-game-id="${escapeHtml(game.id)}">
          <div class="storage-item-header">
            <span class="storage-item-name">${escapeHtml(game.name)}</span>
            <span class="storage-item-size">${escapeHtml(game.size)}</span>
          </div>
          <div class="storage-item-meta">
            <span><i class="fas fa-calendar"></i> ${escapeHtml(game.date)}</span>
            <span><i class="fas fa-key"></i> ${game.keys} keys</span>
          </div>
          <div class="storage-item-actions">
            <button class="storage-item-clear" data-game-id="${escapeHtml(game.id)}">
      ...
      el.storageList.querySelectorAll('.storage-item-clear').forEach(btn => {
        btn.addEventListener('click', (e) => {
          const gameId = e.target.closest('.storage-item-clear').dataset.gameId;
          if (confirm(`Clear saved data for "${gameId}"?`)) {
  ...
  function clearAllStorage() {
    if (!confirm('Clear all saved game data? This cannot be undone.')) return;
  ...
  function initFpsPanel() {
    if (!el.fpsBtnToggle || !el.fpsPanel) {
      console.log('FPS panel elements not found, skipping initFpsPanel');
      return;
    }
  ...
  function initShaderPanel() {
    if (!el.shaderBtnToggle || !el.shaderPanel) {
      console.log('Shader panel elements not found, skipping initShaderPanel');
      return;
    }
    // toggle
...
(function() {
  ...
  function initModPanel() {
    if (!el.modBtnToggle || !el.modPanel) {
      console.log('Mod panel elements not found, skipping initModPanel');
      return;
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // LOCALSTORAGE BRIDGE — persist game data across sessions
  // ─────────────────────────────────────────────────────────────────────────────
  const STORAGE_PREFIX = 'tkhub_game_';
  const STORAGE_VERSION = '1.0';

  // Parent storage operations
  function getGameStorage(gameId) {
    try {
      const key = `${STORAGE_PREFIX}${gameId}`;
      const data = localStorage.getItem(key);
      return data ? JSON.parse(data) : {};
    } catch (e) {
      console.warn('Failed to get game storage:', e);
      return {};
    }
  }

  function setGameStorage(gameId, data) {
    try {
      const key = `${STORAGE_PREFIX}${gameId}`;
      const storageData = {
      ...
      console.warn('Failed to set game storage:', e);
      return false;
    }
  }

  function clearGameStorage(gameId) {
    try {
      const key = `${STORAGE_PREFIX}${gameId}`;
...
let lastRequiredVersion = '';
let updateCheckInterval = null;

// Start automatic saving for current game
  function startAutoSave() {
    ...
    console.log('[AutoSave] Started for game:', state.currentGame.filename);
  }
...
// Save game localStorage using polling approach
  function saveGameStorage() {
    try {
      if (!state.currentGame) return;
      
      const gameId = state.currentGame.filename;
      
      // Try to access iframe's localStorage directly
      try {
        const cw = el.gameFrame.contentWindow;
        if (cw && cw.localStorage) {
          const storageData = {};
          for (let i = 0; i < cw.localStorage.length; i++) {
            const key = cw.localStorage.key(i);
            storageData[key] = cw.localStorage.getItem(key);
          }
          
          if (Object.keys(storageData).length > 0) {
            setGameStorage(gameId, storageData);
            console.log('[Storage] Auto-saved', Object.keys(storageData).length, 'keys for game:', gameId);
          }
        }
      ...
      console.warn('[Storage] Failed to save game storage:', e);
    }
  }

// Load game localStorage when game opens
  function loadGameStorage() {
    try {
      const cw = el.gameFrame.contentWindow;
      if (!cw || !state.currentGame) return;

      const gameId = state.currentGame.filename;
      const storage = getGameStorage(gameId);

      if (Object.keys(storage).length > 0) {
        // Try to set localStorage directly
        try {
          Object.keys(storage).forEach(key => {
            cw.localStorage.setItem(key, storage[key]);
          });
          console.log('[Storage] Loaded', Object.keys(storage).length, 'keys for game:', gameId);
        } catch (crossOriginError) {
      ...
      console.warn('[Storage] Failed to load game storage:', e);
    }
  }

// Inject storage handler into iframe
  function injectStorageBridge() {
    try {
      const cw = el.gameFrame.contentWindow;
      if (!cw) return;

      // Inject script to handle storage communication and proxy localStorage
      const storageScript = `
        (function() {
          // Storage proxy to intercept all localStorage calls
          ...
          window.addEventListener('message', (e) => {
            ...
            } else if (e.data.type === 'setLocalStorage') {
              const data = e.data.data || {};
              Object.keys(data).forEach(key => {
                localStorage.setItem(key, data[key]);
              });
              console.log('[Storage] Loaded', Object.keys(data).length, 'keys from parent');
            }
          });
          
          // Listen for storage messages from iframes
          window.addEventListener('message', (e) => {
            if (e.data.type === 'localStorageData') {
              const { gameId, data, timestamp } = e.data;
              if (gameId && data && state.currentGame && state.currentGame.filename === gameId) {
                setGameStorage(gameId, data);
                console.log('[Storage] Received data from iframe:', Object.keys(data).length, 'keys for game:', gameId);
              }
            } else if (e.data.type === 'localStorageChanged') {
              const { action, gameId, timestamp } = e.data;
              console.log('[Storage] iframe localStorage changed:', action, gameId);
              // Trigger a save on any localStorage change
              if (state.currentGame) {
                setTimeout(() => saveGameStorage(), 100);
              }
            }
          });

          console.log('[Storage Bridge] Injected successfully');
        })();
      `;

      // Inject via blob URL to avoid eval restrictions
      const blob = new Blob([storageScript], { type: 'text/javascript' });
      const url = URL.createObjectURL(blob);
      ...
      console.log('[Storage Bridge] Injected for game:', state.currentGame?.filename);
    } catch (e) {
      console.warn('Failed to inject storage bridge:', e);
    }
  }
  ...
  function initDevPanel() {
    if (!el.devBtnToggle || !el.devPanel) {
      console.log('Dev panel elements not found, skipping initDevPanel');
      return;
    }
  ...
  function initEventListeners() {
    ...
    el.announcementClose.addEventListener('click', hideAnnouncement);

    // update popup close button
    el.updatePopupClose.addEventListener('click', hideUpdatePopup);

    // storage management
    document.getElementById('manageStorage')?.addEventListener('click', showStorageModal);
    el.storageModalClose.addEventListener('click', hideStorageModal);
    el.storageClearAll.addEventListener('click', clearAllStorage);
